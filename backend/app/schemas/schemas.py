...
class GapCandidateSchema(BaseModel):
    ...
    description: str = Field(..., min_length=10, description="Gap 描述（英文，人类可读摘要；下游检索与审计基于英文语料，中文会降低召回）")
    target_setting: str = Field(..., description="目标场景（英文）")
    observed_problem: str = Field(..., description="在证据中观察到的具体问题（英文）")
    existing_coverage: str = Field(..., description="现有论文已覆盖的内容（英文）")
    missing_capability: str = Field(..., description="缺失的具体能力/方法/数据集（英文）")
    claimed_delta: str = Field(..., description="与现有工作的声称差异（英文，用于生成对抗检索 query）")
    testable_hypothesis: str = Field(..., description="可检验假设（英文，可被实验验证或证伪）")
    falsification_condition: str = Field(..., description="证伪条件（英文，什么证据能证明此 Gap 不成立）")
